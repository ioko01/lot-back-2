            ...
            try {
                ...
                if (authorize) {
                    if (authorize !== 401) {
                        ...
                        const attr = "logo, store_id, s_name, user_create_id"
    ...
    getStoreAll = (url: string, middleware: (req: Request, res: Response, next: NextFunction) => void, roles: TUserRole[]) => {
        router.get(url, middleware, async (req: Request, res: Response) => {
            try {
                ...
                if (authorize) {
                    if (authorize !== 401) {
                        const attr = "stores.store_id AS s_id, u_name, logo, stores.user_create_id AS u_id, fullname"
            ...
            try {
                ...
                if (authorize) {
                    if (authorize !== 401) {
                        ...
                        const attr = "store_id, s_name, logo, user_create_id"
                        const where = [["s_name", "=", data.name]]
                        ...
                        const attr2 = ["logo", "s_name", "user_create_id", "store_id"]
